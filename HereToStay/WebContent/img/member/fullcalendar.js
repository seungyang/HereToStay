...
//        this.unrenderHighlight(); 지움 sowon
...
//        this.callChildren('unrenderSelection', arguments);
    ...
    DateComponent.prototype.getDayClasses = function (date, noThemeHighlight) {
        ...
        var classes = [];
        var today;
        var dateYYYY= date.format().substring(0,4);
        var dateMM =date.format().substring(5,7);
        if(dateMM.substring(0,1)==0)
        	dateMM=dateMM.substring(1,2);
        var dateDD =date.format().substring(8,10);
        if(dateDD.substring(0,1)==0)
        	dateDD=dateDD.substring(1,2);
        var compareDate =new Date(Number(dateYYYY),Number(dateMM)-1,Number(dateDD));
        
        var shot= $('#shot').val();
        var sy=shot.substring(0,4);
        var sm =shot.substring(5,7);
        if(sm.substring(0,1)==0)
        	sm=sm.substring(1,2);
        var sd=shot.substring(8,10);
        if(sd.substring(0,1)==0)
        	sd=sd.substring(1,2);
        var shotCompare=new Date(Number(sy),Number(sm)-1,Number(sd));
        
        var ehot= $('#ehot').val();
        var ey=ehot.substring(0,4);
        var em =ehot.substring(5,7);
        if(em.substring(0,1)==0)
        	em=em.substring(1,2);
        var ed=ehot.substring(8,10);
        if(ed.substring(0,1)==0)
        	ed=ed.substring(1,2);
        var ehotCompare=new Date(Number(ey),Number(em)-1,Number(ed));
        ...
        if (!this.dateProfile.activeUnzonedRange.containsDate(date)) {
            ...
            if (date.isSame(today, 'day')) {
                ...
                var cIn= $('#checkIn').val();
                var y=cIn.substring(6,10);
                var m =cIn.substring(0,2);
                if(m.substring(0,1)==0)
                	m=m.substring(1,2);
                var d=cIn.substring(3,5);
                if(d.substring(0,1)==0)
                	d=d.substring(1,2);
                var compareIndate=new Date(Number(y),Number(m)-1,Number(d));
             
                var Y= today.format().substring(0,4);
                var M =today.format().substring(5,7);
                if(M.substring(0,1)==0)
                	M=M.substring(1,2);
                var D =today.format().substring(8,10);
                if(D.substring(0,1)==0)
                	D=D.substring(1,2);
                var compareToday =new Date(Number(Y),Number(M)-1,Number(D));                
                
                if(compareIndate.getTime()<=compareToday.getTime()){
                	
                	classes.push('fc-highlight');
                } 
                ...
                var yy=checkIn.substring(6,10);
                var mm =checkIn.substring(0,2);
                if(mm.substring(0,1)==0)
                	mm=mm.substring(1,2);
                var dd=checkIn.substring(3,5);
                if(dd.substring(0,1)==0)
                	dd=dd.substring(1,2);
                var indate=new Date(Number(yy),Number(mm)-1,Number(dd));
                
                var checkOut= $('#out').val();
                var yy2=checkOut.substring(6,10);
                var mm2 =checkOut.substring(0,2);
                if(mm2.substring(0,1)==0)
                	mm2=mm2.substring(1,2);
                var dd2=checkOut.substring(3,5);
                if(dd2.substring(0,1)==0)
                	dd2=dd2.substring(1,2);
                var outdate=new Date(Number(yy2),Number(mm2)-1,Number(dd2));                
             
           //        
               if(outdate.getTime()>=compareDate.getTime()&&indate.getTime()<=compareDate.getTime())
             {
            	   classes.push('fc-highlight');
        ...
        if(shotCompare.getTime()<=compareDate.getTime() && compareDate.getTime()<=ehotCompare.getTime() )
        {
        	classes.push('fc-hot');
        }
        return classes;
...
//                selectionFootprint = null;
            ...
            dragStart: function (ev) {
                ...
                $('.cal li a').removeClass('active');
    			$('#viewRooms').addClass('active');
    			$('.content-tabs-i').css("display","none");
    			$('#selCal').css("display",'block');
            },
            hitOver: function (hit, isOrig, origHit) {
...
//        this.unrenderHighlight(); //지워지는거 예방ㅇㅇㄴㅇㅇㅇ
//        this.helperRenderer.unrender();
    };
    /* Event Resize Visualization
    ------------------------------------------------------------------------------------------------------------------*/
