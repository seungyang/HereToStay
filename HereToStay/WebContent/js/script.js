$(document).ready(function(){
	...
	if ($wid > 900) {
		...
		$('.wrapper-padding_mobile').hide();
	}
	else
	{
		$('.wrapper-padding_mobile').show();
		$('.wrapper-padding_mobile').prev().hide();
	}
    init_validation();
	...
	var $headerDown = function() {
		...
		header_logo.find('img').attr('src','/img/logo3.png');
...
//        		$('#loginOk').attr('action',"/member/index_member.do?session=" + $('#logId').val());
					$('#loginOk').submit();
	        		alert('로그인 성공');
	        	}
			}
	...
	$(document).on('click','.gallery-i a',function(){
	...
	$('.content-tabs-head a').on('click',function(){
		...
		if($index==1 )
		{
			$('.h-tab-i').removeClass('active');
			 $('.h-tab-i').eq(3).addClass('active');
		$('.tab-item').css("display",'none');
		$('#fullCal').css("display",'block');
...
//		  
		 ...
		 addCategoryClickEvent();
			...
			// 엘리먼트에 이벤트 핸들러를 등록하는 함수입니다
			 ...
			 function addMarker(position, order) {
			     var imageSrc = '/img/member/download1.png', // 마커 이미지 url, 스프라이트 이미지를 씁니다
			         imageSize = new daum.maps.Size(27, 28),  // 마커 이미지의 크기
			         imgOptions =  {
			             spriteSize : new daum.maps.Size(72, 243), // 스프라이트 이미지의 크기
	...
	$(window).on('resize',function(){
		...
		if ($width > 900) {
			...
			$('.wrapper-padding_mobile').hide();
			$('.wrapper-padding_mobile').prev().show();
		}
		else
		{
			$('.wrapper-padding_mobile').show();
