$(document).ready(function(){
 ...
 // 시작
	$('.header-account a').on('click',function(){
		...
		window.location.replace("/member/index_member.do");
	...
	// 관리자
	$('.header-account3 a').on('click',function(){
		$('.overlay3').fadeIn(function(){
			$('.autorize-popup3').animate({top: '50%'}, 300).find('input:text').eq('0').focus();
		});
		return false;
	});
	...
	$('.overlay3').on('click',function(){
		$('.autorize-popup3').animate({top: '-300px'}, 300, function(){
			$('.overlay3').fadeOut();	
		});
	});
	...
	//관리자
	$('.autorize-tab-content3').css('display','block');
	...
	//관리자
	$('.autorize-tabs3 a').on('click',function(){
		if ( $(this).is('.autorize-close') ) {
			$('.autorize-popup3').animate({top: '-300px'}, 300, function(){
				$('.overlay3').fadeOut();	
			});
